...
export const api = {
  ...
  fetchDashboardMetrics: async (days: number = 1): Promise<StatMetric[]> => {
    ...
    const prevPeriodStartStr = new Date(periodStart);
    prevPeriodStartStr.setDate(prevPeriodStartStr.getDate() - days);
    ...
    try {
      const [
        ...
        avgResponseResult,
        // New: appointments for show rate
        appointmentsCompletedResult,
        appointmentsNoShowResult,
      ] = await Promise.all([
        supabase
          .from('conversations')
        ...
        supabase
          .from('conversations')
          .select('id', { count: 'exact', head: true })
          .gte('last_message_at', prevPeriodStartStr.toISOString())
        ...
        supabase
          .from('contacts')
          .select('id', { count: 'exact', head: true })
          .gte('created_at', prevPeriodStartStr.toISOString())
        ...
        supabase
          ...
          .select('id', { count: 'exact', head: true })
          .not('won_at', 'is', null)
          .gte('won_at', prevPeriodStartStr.toISOString())
      ...
      // Scheduling rate: appointments / leads
      const schedulingRate = contactsPeriod > 0 
        ? Math.round(((appointmentsPeriodResult.count || 0) / contactsPeriod) * 100) 
        : 0;
      const schedulingRatePrev = contactsPrev > 0
        ? Math.round(((appointmentsPrevResult.count || 0) / contactsPrev) * 100)
        : 0;
      ...
      return [
        ...
        {
          label: 'Agendamentos',
          value: (appointmentsPeriodResult.count || 0).toString(),
          trend: calculateTrend(appointmentsPeriodResult.count || 0, appointmentsPrevResult.count || 0),
          trendUp: (appointmentsPeriodResult.count || 0) >= (appointmentsPrevResult.count || 0)
        },
        {
          label: 'Taxa Agendamento',
          value: `${schedulingRate}%`,
          trend: calculateTrend(schedulingRate, schedulingRatePrev),
          trendUp: schedulingRate >= schedulingRatePrev
        },
        {
          label: 'Conversões',
      ...
      return [
        ...
        { label: 'Taxa Agendamento', value: '0%', trend: '0%', trendUp: true },
  ...
  deleteTeamMember: async (id: string): Promise<void> => {
    ...
    logAuditEvent('member.deleted', 'team_member', id);
  },
  ...
  fetchPipeline: async (): Promise<Deal[]> => {
    ...
    return (data || []).map((d: any) => ({
      ...
      qualificationScore: d.qualification_score ?? (d.contact?.client_memory?.lead_profile?.qualification_score || 0),
    }));
  },
  ...
  moveDealStage: async (id: string, newStageId: string): Promise<void> => {
    ...
    logAuditEvent('pipeline.deal_moved', 'deal', id, { new_stage_id: newStageId });
  },

  /**
   * Update deal qualification score
   */
  updateDealScore: async (dealId: string, score: number, criteria?: Record<string, number>): Promise<void> => {
    const { error } = await supabase
      .from('deals')
    ...
    if (error) {
      console.error('[API] Error updating deal score:', error);
      throw error;
    }

    logAuditEvent('deal.score_updated', 'deal', dealId, { score, criteria });
  },
