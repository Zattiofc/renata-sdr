              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
                ...
                <div className="space-y-4">
                  <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-2">
                    ...
                    {activeChat.clientMemory.interaction_summary.total_conversations > 0 && (
                      <span className="ml-auto px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 text-[10px] font-semibold flex items-center gap-1">
                  ...
                  {activeChat.resumoVivo && (
                    <div className="p-3 rounded-lg bg-primary/5 border border-primary/20">
                      <span className="text-xs text-primary font-semibold flex items-center gap-1.5 mb-1.5">
                  ...
                  {(activeChat.empresa || activeChat.cargo || activeChat.cidade || activeChat.linhaNegocio) && (
                    <div className="p-3 rounded-lg bg-muted/50 border border-border space-y-1">
                      <span className="text-xs text-muted-foreground font-semibold">Perfil do Lead</span>
                      {activeChat.empresa && <p className="text-xs text-foreground">🏢 {activeChat.empresa}</p>}
                      {activeChat.cargo && <p className="text-xs text-foreground">💼 {activeChat.cargo}</p>}
                      {activeChat.cidade && <p className="text-xs text-foreground">📍 {activeChat.cidade}{activeChat.estado ? ` - ${activeChat.estado}` : ''}</p>}
                      {activeChat.linhaNegocio && <p className="text-xs text-foreground">🏥 Linha: {activeChat.linhaNegocio}</p>}
                    </div>
                  )}
                  ...
                  {(activeChat.clientMemory.sales_intelligence as any).objections?.length > 0 && (
                    <div className="p-3 rounded-lg bg-orange-500/10 border border-orange-500/20">
                      <span className="text-xs text-orange-400">Objeções</span>
                      <p className="text-sm text-foreground mt-1">
                        {(activeChat.clientMemory.sales_intelligence as any).objections.join(', ')}
                      </p>
                    </div>
                  )}
                  ...
                  <div className="p-3 rounded-lg bg-muted/50 border border-border">
                    ...
                    <p className="text-sm text-foreground mt-1">
                      {activeChat.clientMemory.sales_intelligence.next_best_action === 'qualify' ? 'Qualificar lead' :
                       ...
                       activeChat.clientMemory.sales_intelligence.next_best_action === 'followup' ? 'Follow-up' :
                       activeChat.clientMemory.sales_intelligence.next_best_action === 'close' ? 'Fechar negócio' :
                       activeChat.clientMemory.sales_intelligence.next_best_action === 'nurture' ? 'Nutrir lead' :
                        ...
                        <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                          <div 
                            className={`h-full rounded-full transition-all ${
                              activeChat.clientMemory.lead_profile.qualification_score > 70 ? 'bg-emerald-500' :
                              activeChat.clientMemory.lead_profile.qualification_score > 40 ? 'bg-amber-500' : 'bg-red-500'
                            }`}
                            style={{ width: `${activeChat.clientMemory.lead_profile.qualification_score}%` }}
                          />
                        </div>
                        <span className="text-sm font-bold text-foreground">{activeChat.clientMemory.lead_profile.qualification_score}</span>
                      </div>
                    </div>
                  )}
