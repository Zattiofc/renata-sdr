...
import { Activity, DollarSign, MessageSquare, Users, Loader2, TrendingUp, TrendingDown, ArrowUpRight, CalendarCheck, Percent, Clock } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const getIcon = (label: string) => {
    ...
    if (label.includes('Agendamentos')) return <CalendarCheck className="h-5 w-5 text-blue-600" />;
    if (label.includes('Taxa')) return <Percent className="h-5 w-5 text-amber-600" />;
    if (label.includes('Tempo')) return <Clock className="h-5 w-5 text-orange-600" />;
  ...
  const getGradient = (label: string) => {
    ...
    if (label.includes('Agendamentos')) return 'from-blue-500/20 to-blue-500/5 border-blue-500/20';
    if (label.includes('Taxa')) return 'from-amber-500/20 to-amber-500/5 border-amber-500/20';
