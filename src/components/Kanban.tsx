...
import { 
  ...
  FileText, Phone, Mail, Paperclip, Send, CheckSquare, Clock, Trash2, Settings, Brain, MessageSquare, Bot, AlertTriangle, Target
} from 'lucide-react';
import { Button } from './Button';
import { api } from '../services/api';
import { Deal, DealActivity, TeamMember, KanbanColumn } from '../types';
import { QualificationScoreEditor } from './QualificationScoreEditor';
...
const Kanban: React.FC = () => {
  ...
  const onDrop = async (e: React.DragEvent, targetStageId: string) => {
    ...
    // Transition validation: warn if score is low and moving forward
    if (deal && targetStageIndex > currentStageIndex + 1 && (deal.qualificationScore ?? 0) < 40) {
      const confirm = window.confirm(
        `⚠️ Score de qualificação baixo (${deal.qualificationScore ?? 0}%).\n\nTem certeza que deseja avançar "${deal.title}" para "${targetStage?.title}"?\n\nRecomendação: qualifique o lead antes de avançar.`
      );
      if (!confirm) return;
    }
    ...
    const updatedDeals = deals.map(d => {
      if (d.id === dealId) {
        return { ...d, stageId: targetStageId };
      }
      return d;
    });
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {(deal.qualificationScore ?? 0) > 0 && (
                          <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold border ${
                            (deal.qualificationScore ?? 0) >= 70 
                              ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' 
                              : (deal.qualificationScore ?? 0) >= 40 
                ...
                <div className="flex-1 overflow-y-auto bg-background custom-scrollbar">
                    ...
                    {selectedDeal.clientMemory && (
                      <div className="p-6 border-t border-border">
                        ...
                        <div className="space-y-3">
                          {/* Editable Qualification Score */}
                          <div className="p-3 rounded-lg bg-card border border-border">
                            <QualificationScoreEditor
                              currentScore={selectedDeal.qualificationScore ?? selectedDeal.clientMemory?.lead_profile?.qualification_score ?? 0}
                              onSave={async (score, criteria) => {
                                try {
                                  await api.updateDealScore(selectedDeal.id, score, criteria);
                                  setSelectedDeal({ ...selectedDeal, qualificationScore: score });
                                  setDeals(deals.map(d => d.id === selectedDeal.id ? { ...d, qualificationScore: score } : d));
                                  toast.success(`Score atualizado para ${score}%`);
                                } catch {
