...
export function transformDBToUIConversation(
  ...
  return {
    ...
    // Persistent memory fields
    resumoVivo: (conv.contact as any)?.resumo_vivo || null,
    empresa: (conv.contact as any)?.empresa || null,
    cargo: (conv.contact as any)?.cargo || null,
    cidade: (conv.contact as any)?.cidade || null,
    estado: (conv.contact as any)?.estado || null,
    linhaNegocio: (conv.contact as any)?.linha_negocio || null,
  };
}
