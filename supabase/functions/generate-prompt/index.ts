...
function cleanGeneratedPrompt(raw: string): string {
  ...
  if (xmlStart > 0) cleaned = cleaned.substring(xmlStart);
  const xmlEnd = cleaned.lastIndexOf('</system_instruction>');
  if (xmlEnd > 0) cleaned = cleaned.substring(0, xmlEnd + '</system_instruction>'.length);
  cleaned = cleaned.replace(
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!aiConfig.apiKey) {
      return new Response(
        JSON.stringify({ error: 'API Key do provedor de IA não configurada.' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
  ...
  <knowledge_base>
    <products>[LISTA_DE_PRODUTOS_E_REGRAS]</products>
    <differentials>[LISTA_DE_DIFERENCIAIS_COMPETITIVOS]</differentials>
  </knowledge_base>
  ...
  <guidelines>
    ...
    <conversation_flow>
      ...
      2. **Descoberta (Prioridade Máxima):** Motivação + Qualificação Técnica.
      3. **Compromisso:** Se qualificado -> [AÇÃO_DE_CONVERSÃO].
    </conversation_flow>
  </guidelines>
  ...
  <cognitive_process>
    1. **Analyze:** Etapa do funil?
    2. **Check:** O que falta descobrir?
    3. **Plan:** Melhor pergunta aberta?
    4. **Draft & Refine:** Escreva e corte.
    5. **Validate:** Tom adequado?
  </cognitive_process>
...
Sua tarefa é preencher o template com as informações fornecidas, mantendo a estrutura XML.
...
3. Para listas, formate como bullet points
4. Para MAX_LINES_ABSOLUTE, use o dobro do MAX_LINES

🚨 REGRAS ESPECIAIS PARA VARIÁVEIS DINÂMICAS:
8. USE EXATAMENTE estas variáveis no formato {{ nome }}:
   - {{ data_hora }}, {{ data }}, {{ hora }}, {{ dia_semana }}, {{ cliente_nome }}, {{ cliente_telefone }}
9. PROIBIDO usar DateTime.now() ou qualquer código JavaScript/Luxon
10. Retorne APENAS o XML, sem texto introdutório ou blocos de código markdown
...
Gere o prompt completo preenchido:`;

    console.log('[generate-prompt] Calling external AI...');
    ...
    if (!response.ok) {
      ...
      console.error('[generate-prompt] AI error:', response.status, errorText);
