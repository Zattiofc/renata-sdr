...
serve(async (req) => {
  ...
  // Get AI config from nina_settings for transcription
...
async function combineAndTranscribeMessages(
  ...
  for (let i = 0; i < queueMessages.length; i++) {
    ...
    if (messageData.type === 'audio' || messageData.messageType === 'audio') {
      ...
      if (audioMediaId && instanceId && aiConfig.apiKey) {
        ...
        if (mediaResult) {
          ...
          const transcription = await transcribeAudio(mediaResult.buffer, aiConfig);
          if (transcription) {
      ...
      // Fall back to WhatsApp Official API if Evolution failed or no instance_id
      else if (audioMediaId && settings?.whatsapp_access_token && aiConfig.apiKey) {
        ...
        if (audioBuffer) {
          const transcription = await transcribeAudio(audioBuffer, aiConfig);
          if (transcription) {
            content = transcription;
            await supabase
              .from('messages')
...
// Transcribe audio using configured AI provider (Gemini with audio input)
async function transcribeAudio(audioBuffer: ArrayBuffer, aiConfig: any): Promise<string | null> {
  try {
    ...
    // For Google Gemini, use multimodal input_audio format
    ...
    // For OpenAI, use the audio transcription endpoint
    if (aiConfig.provider === 'openai') {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('[MessageGrouper] OpenAI transcription error:', response.status, errorText);
        return null;
      }
    ...
    // For Anthropic or others, skip transcription (not supported)
    console.log('[MessageGrouper] Audio transcription not supported for provider:', aiConfig.provider);
    return null;
  } catch (error) {
