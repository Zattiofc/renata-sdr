...
serve(async (req) => {
  ...
  try {
    ...
    let emptyChecks = 0;
    const MAX_EMPTY_CHECKS = 3; // Allow up to 3 empty checks before exiting (grace period for race conditions)
    ...
    while (Date.now() - startTime < MAX_EXECUTION_TIME) {
      ...
      if (!queueItems || queueItems.length === 0) {
        ...
        // Check for messages scheduled in the next 10 seconds
        const { data: upcoming, error: upcomingError } = await supabase
          .from('send_queue')
          .select('id, scheduled_at')
          .eq('status', 'pending')
          .lte('scheduled_at', new Date(Date.now() + 10000).toISOString())
        ...
        if (upcoming && upcoming.length > 0) {
          ...
          if (waitTime > 0 && (Date.now() - startTime + waitTime) < MAX_EXECUTION_TIME) {
            ...
            emptyChecks = 0; // Reset since we found scheduled messages
            continue;
          }
        }
        
        // Grace period: wait briefly before giving up (handles race condition where messages are being inserted)
        emptyChecks++;
        if (emptyChecks < MAX_EMPTY_CHECKS && (Date.now() - startTime + 2000) < MAX_EXECUTION_TIME) {
          console.log(`[Sender] Empty check ${emptyChecks}/${MAX_EMPTY_CHECKS}, waiting 2s grace period...`);
      ...
      // Reset empty checks when we find messages
      emptyChecks = 0;
